...
function BarGroup(props) {
    let barPadding = 2
    let barColour = '#7BD24F'
    let widthScale = d => d * 0.08
    let dateFormat = date => {
    ...
    let width = widthScale(props.d.recovered)
    let yMid = props.barHeight * 0.5
    
    return <g className="bar-group">
      <text className="name-label" x="-6" y={yMid} alignmentBaseline="middle" >{dateFormat(props.d.date)}</text>
      <rect y={barPadding * 0.5} width={width} height={props.barHeight - barPadding} fill={barColour} />
      <text className="value-label" x={width- 8} y={yMid} alignmentBaseline="middle" >{props.d.recovered}</text>
    </g>
  }
    ...
    render() {
      ...
      return <div className="containerData activeCasesBox"><svg width="300" height="275">
        <g className="container">
          <text className="title" x="10" y="30">Recovered Cases</text>
